    ...
    public class ItemPedidoController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? ped)
        {
            if (ped.HasValue)
            {
                ...
                {
                    var pedido = await _context.Pedidos.Include(p => p.Cliente).Include(p => p.ItensPedido.OrderBy(i => i.Produto.Nome))
                    .ThenInclude(i => i.Produto).FirstOrDefaultAsync(p => p.IdPedido == ped);
        ...
        public async Task<IActionResult> Cadastrar([FromForm] ItemPedidoModel itemPedido)
        {
            if (ModelState.IsValid)
            {
                if (itemPedido.IdPedido > 0)
                {
                    ...
                    pedido.ValorTotal = _context.ItensPedidos.Where(i => i.IdPedido == itemPedido.
                        IdPedido).Sum(i => i.ValorUnitario * i.Quantidade);
        ...
        public async Task<IActionResult> Excluir(int? ped, int? prod)
        {
            ...
            {
                TempData["mensagem"] = MensagemModel.Serializar("Item de pedido não foi informado.", TipoMensagem.Erro);
                return RedirectToAction("Index", "Cliente");
            }
