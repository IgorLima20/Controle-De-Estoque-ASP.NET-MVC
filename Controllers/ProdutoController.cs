...
namespace EstoqueWeb.Controllers 
{
    public class ProdutoController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                if (id.HasValue)
                {
                    if (ProdutoExiste(id.Value))
                    {
                        ...
                        {
                            TempData["mensagem"] = MensagemModel.Serializar("Erro ao alterar o produto.", TipoMensagem.Erro);
                        }
                    }
                    else
                    {
        ...
        public async Task<IActionResult> Excluir(int? id)
        {
            if (!id.HasValue)
            {
                TempData["mensagem"] = MensagemModel.Serializar("Produto não foi informado.", TipoMensagem.Erro);
                return RedirectToAction("Index");
            }
