    ...
    public class PedidoController : Controller
    {
        ...
        public async Task<IActionResult> Index(int? cid)
        {
            if (cid.HasValue)
            {
                ...
                if (cliente != null)
                {
                    var pedidos = await _context.Pedidos.Where(p => p.IdCliente == cid).OrderByDescending(x => x.IdPedido).AsNoTracking().ToListAsync();
                    ViewBag.Cliente = cliente;
                    return View(pedidos);
                }
                else
                {
                    TempData["mensagem"] = MensagemModel.Serializar("Cliete não encontrado.", TipoMensagem.Erro);
                    return RedirectToAction("Index", "Clientes");
                }
            }
            else
            {
                TempData["mensagem"] = MensagemModel.Serializar("Cliete não informado", TipoMensagem.Erro);
                return RedirectToAction("Index", "Clientes");
            }
        }
        ...
        public async Task<IActionResult> Excluir(int? id)
        {
            if (!id.HasValue)
            {
                TempData["mensagem"] = MensagemModel.Serializar("Pedido não foi informada.", TipoMensagem.Erro);
                return RedirectToAction("Index");
            }
